...
class TissueConfigurator(object):
    # Rename stop_in_nuclei -> seed properties.
...
class GmTissue(TissueConfigurator):
    ...
    def can_continue(self, line, mask):
        ...
        for i in line[-self.max_consecutive_steps :]:
            history.append(mask.voxmm_to_value(*i, origin="corner"))
        if len(np.unique(history)) == 1:
            return False

        value = mask.voxmm_to_value(
            *line[-1], origin="corner", interpolator="trilinear"
        )
    ...
    def finalize_streamline(self, line, last_dir, tracker):
        if self.is_valid_endpoint():
            ...
            for i in range(nb_ending_steps):
                ...
                new_pos, new_dir, _ = tracker.propagator.propagate(line[-1], last_dir)
                line.append(new_pos)
                last_dir = new_dir

                new_id = int(
                    tracker.mask.voxmm_to_value(*line[-1], origin=tracker.origin)
                )
                if new_id != id:
                    line.pop()
                    break
    ...
    def finalize_streamline(self, line, last_dir, tracker):
        if self.is_valid_endpoint():
            ...
            if self.max_nb_ending_steps:
                ...
                nb_ending_steps = 1
            vox_idx = tuple(
                tracker.mask.voxmm_to_idx(*line[-1], tracker.origin).astype(np.int)
            )
