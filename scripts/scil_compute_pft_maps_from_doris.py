...
def _build_arg_parser():
    ...
    p.add_argument(
        ...
        help='Output interface mask (nifti). [interface.nii.gz]')
    p.add_argument(
...
def main():
    ...
    interface = np.zeros(wm_pve.shape, dtype=np.uint8)
    interface[gm_pve >= args.int_thres] = 1
    interface[gm_pve < 0.05] = 0
    interface[wm_pve < args.int_thres] = 0
    ...
    nib.Nifti1Image(interface.astype('float32'),
                    segmentation_img.affine).to_filename(args.interface)
